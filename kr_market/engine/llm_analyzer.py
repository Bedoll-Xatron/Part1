...
class NvidiaAnalyzer:
    def __init__(self):
        ...
        try:
            ...
            print(f"[NvidiaAnalyzer] 초기화 실패: {e}")
            self.client = None
    ...
    async def analyze_news(self, stock_name: str, news_items: List[Dict]) -> Dict:
        ...
        # 초기화 실패 → 바로 키워드 분석
        if not self.client:
            print(f"[NvidiaAnalyzer] client 없음, 키워드 분석 사용 ({stock_name})")
...
{{"score": 2, "reason": "이유 한 줄", "themes": ["테마1", "테마2"]}}"""
        ...
        try:
            ...
            print(f"[NvidiaAnalyzer] 타임아웃 60초 초과, 키워드 분석으로 대체 ({stock_name})")
            result = self._fallback_keyword_analysis(news_items)
            result["source"] = "keyword_fallback"
            return result

        except json.JSONDecodeError as e:
            print(f"[NvidiaAnalyzer] JSON 파싱 실패, 키워드 분석으로 대체 ({stock_name}): {e}")
            result = self._fallback_keyword_analysis(news_items)
            result["source"] = "keyword_fallback"
            return result

        except Exception as e:
            print(f"[NvidiaAnalyzer] 에러, 키워드 분석으로 대체 ({stock_name}): {e}")
            result = self._fallback_keyword_analysis(news_items)
            result["source"] = "keyword_fallback"
            return result

    def _fallback_keyword_analysis(self, news_items: List[Dict]) -> Dict:
        """키워드 기반 백업 분석 (LLM 실패 시 사용)."""
        if not news_items:
...
async def run_news_analysis(stocks: List) -> List[Dict]:
    """필터링된 후보 종목의 뉴스를 수집하고 LLM으로 분석한다.
    ...
    print("=" * 70)
    print(f"  뉴스 기반 LLM 분석 시작 ({len(stocks)}개 종목)")
    print("=" * 70)
